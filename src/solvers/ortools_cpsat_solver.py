    ...
    def solve(self, time_limit=30):
        ...
        for job_id, job in enumerate(self.jobs):
            for op_idx, (machine_id, duration) in enumerate(job):
                ...
                all_tasks[(job_id, op_idx)] = (start, end, interval, machine_id, duration)
        ...
        for job_id, job in enumerate(self.jobs):
            for op_idx in range(len(job) - 1):
                prev_end = all_tasks[(job_id, op_idx)][1]  # tempo final da operação atual
                next_start = all_tasks[(job_id, op_idx + 1)][0]  # tempo inicial da próxima operação
                model.Add(next_start >= prev_end)
        
        # Define a variável makespan
        makespan = model.NewIntVar(0, horizon, "makespan")
        all_ends = [all_tasks[(job_id, len(job) - 1)][1] for job_id, job in enumerate(self.jobs)]
        ...
        if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
            for job_id, job in enumerate(self.jobs):
                for op_idx, (machine_id, duration) in enumerate(job):
                    ...
                    schedule.add_operation(job_id, op_idx, machine_id, start_time, duration)
