...
class VNDLocalSearch(LocalSearchStrategy):
    ...
    def __init__(self, fitness_func: Callable[[list], float],
        ...
        Args:
            ...
            thread_id: Identificador para esta instância de VND (para shared pool).
            shared_solution_pool: Instância do pool de soluções compartilhado.
            share_frequency: A cada quantas iterações VND compartilhar/receber soluções.
            reactive_update_N: A cada quantas aplicações de operador atualizar pesos reativos.
            reactive_learning_eta: Taxa de aprendizado (eta) para o gradient bandit.
        """
    ...
    # --- REMOVIDOS: Métodos de Operador de Vizinhança Simples ---
    # _apply_swap, _apply_inversion, _apply_scramble, _2opt, _3opt,
    ...
    def _apply_lns_shake(self, chrom: List[Tuple[int, int]]) -> List[Tuple[int, int]]:
        ...
        for i, (job_id, op_id) in enumerate(ops_to_shake):
            # Acesso seguro a jobs_data
            if job_id >= len(self.jobs_data) or op_id >= len(self.jobs_data[job_id]):
                logger.error(f"LNS Shake: Índice inválido ({job_id}, {op_id}) acessado em jobs_data.")
                return chrom # Retorna original em caso de erro
    ...
    def _apply_critical_lns(self, chrom: List[Tuple[int, int]]) -> List[Tuple[int, int]]:
        ...
        completion_times, critical_path, makespan = calculate_schedule_and_critical_path(
            chrom, self.jobs_data, self.num_machines)
        if not critical_path or len(critical_path) < 2:
            return chrom
    ...
    def local_search(self, chromosome: List[Tuple[int, int]], use_advanced: Optional[bool] = None) -> List[Tuple[int, int]]:
        ...
        if not self.use_orchestrator:
            active_neighborhoods_std = list(self.operator_map.keys())
        ...
        if self.initial_shake_type:
            ...
            if shake_operator_method:
                logger.debug(f"    Aplicando Shake Inicial do tipo: {self.initial_shake_type.name}")
                ...
                if shaken_chrom != best_chrom:
                    ...
                    logger.debug("    Cromossomo modificado pelo Shake Inicial.")
                else:
        ...
        try:
            ...
            logger.debug(f"    [VND Start] Initial Fitness: {best_fit:.2f}")
        except Exception as e:
        ...
        while keep_searching:
            ...
            if self.use_orchestrator:
                ...
                if not hasattr(self, 'neighborhood_stats'):
                     logger.error("VND Padrão selecionado, mas neighborhood_stats não inicializado.")
                     return chromosome

                active_neighborhoods_std = list(self.neighborhood_stats.keys())
                for nh_type_std_ in active_neighborhoods_std:
                    stats = self.neighborhood_stats[nh_type_std_]
                    if stats['attempts'] > 0:
                        stats['success_rate'] = stats['successes'] / stats['attempts']
                    else:
                        stats['success_rate'] = 0.0
                ordered_neighborhoods = sorted(
                    active_neighborhoods_std, key=lambda nh: self.neighborhood_stats[nh]['success_rate'], reverse=True)

                logger.debug(f"      [VND Std Iter {vnd_iterations}] Order: {[nh.name for nh in ordered_neighborhoods]}")
            ...
            if chosen_operator_for_reactive_update:
                ...
                if self.reactive_attempts_count % self.reactive_update_N == 0 and self.orchestrator:
                    # Calcula baseline: recompensa média geral de todos os operadores
                    ...
                    for op_type in self.operator_probabilities:
                        if op_type != chosen_operator_for_reactive_update:
                            prob_other_op = self.operator_probabilities[op_type]
                            delta_p_other = self.reactive_learning_eta * (reward_for_reactive_update - baseline_reward) * prob_other_op
                            self.operator_probabilities[op_type] -= delta_p_other

                    # Garante não-negatividade e renormaliza
                    current_sum_probs = 0.0
                    for op_type in self.operator_probabilities:
                        self.operator_probabilities[op_type] = max(0.001, self.operator_probabilities[op_type]) # min prob para evitar extinção
                        current_sum_probs += self.operator_probabilities[op_type]
                    for op_type in self.operator_probabilities:
                        self.operator_probabilities[op_type] /= current_sum_probs
                    logger.debug(f"Reactive Bandit: Atualizou probabilidades. Ex: {chosen_operator_for_reactive_update.name}={self.operator_probabilities[chosen_operator_for_reactive_update]:.3f}")

            if not improvement_in_iteration:
                non_improving_iterations += 1
                logger.debug(f"      Iteração {vnd_iterations} sem melhoria. Estagnação: {non_improving_iterations}/{self.lns_shake_frequency if self.perform_lns_shake else 'N/A'}")
                ...
                if self.perform_lns_shake and non_improving_iterations >= self.lns_shake_frequency:
                    ...
                    # Meta-UCB: escolhe par (intensity, frequency)
                    if hasattr(self, 'hyper_orch'):
                        arm = self.hyper_orch.pick()
                        new_int, new_freq = arm
                        prev_int, prev_freq = self.lns_shake_intensity, self.lns_shake_frequency
                        self.lns_shake_intensity, self.lns_shake_frequency = new_int, new_freq
                        logger.debug(f"    Meta-UCB selecionou intensity={new_int:.2f}, freq={new_freq}")
                    # guarda fitness pré-shake para calcular recompensa
                    prev_best = best_fit
                    ...
                    if shaken_chrom != best_chrom:
                        ...
                        if shaken_tuple not in evaluated_solutions_cache:
                            try:
                                ...
                                logger.info(f"    [LNS Shake Applied] Novo fitness: {shaken_fit:.2f}")
                                # actualiza reward no hyper_orch
                                if hasattr(self, 'hyper_orch'):
                                    reward = max(0.0, prev_best - shaken_fit)
                                    self.hyper_orch.update(arm, reward)
                                if shaken_fit < best_fit:
                                    ...
                                    logger.info("    LNS Shake diversificou, mas não melhorou.")
                                    best_chrom = shaken_chrom[:]
                                    best_fit = shaken_fit
                            except Exception as e:
                                logger.error(f"Erro ao calcular fitness pós LNS shake: {e}", exc_info=True)
                                logger.warning("    Mantendo cromossomo pré-shake devido a erro.")
                        else:
                            logger.info("    Cromossomo pós-shake já estava no cache.")
                    else:
                ...
                if self.shared_solution_pool and vnd_iterations % self.share_frequency == 0:
                    logger.debug(f"Thread {self.thread_id} tentando compartilhar/receber soluções.")
                    ...
                    if peer_chrom and peer_fitness is not None and peer_fitness < best_fit:
                        ...
                        evaluated_solutions_cache.add(tuple(best_chrom)) # Adiciona ao cache local
        ...
        total_evaluated_count = len(evaluated_solutions_cache)
        logger.debug(
            f"    [VND Final] Tempo: {end_vnd_time - start_vnd_time:.4f}s | Iters: {vnd_iterations} | Soluções Únicas: {total_evaluated_count} | Melhor Fitness: {best_fit:.2f}")
        return best_chrom
